...
export default {
  ...
  methods: {
    ...mapActions("board", ["fetchBoards", "setLockers"]),
    ...mapActions("locker", ["setLocker"]),
    selectedLocker(locker) {
      this.boards[locker.boardNumber].Lockers.forEach((l) => {
        if (l.id === locker.id) l.selected = true;
        else l.selected = false;
      });
      this.setLockers({
        id: locker.boardNumber,
        lockers: this.boards[locker.boardNumber].Lockers,
      });
    },
    isValidSize(locker) {
